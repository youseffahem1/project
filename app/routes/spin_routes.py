...
from .. import models, schemas, ledger_service, game_logic
...
def spin_wheel_preview(
    ...
    - display_segments: the FULL fixed wheel face for the chosen currency
      (WHEEL_DISPLAY_VALUES_NGN or WHEEL_DISPLAY_VALUES_USD) — always the
      same for that currency, regardless of play amount.
    - segments: only the prize/probability pairs that are actually
      ...
      enforced server-side in build_dynamic_prize_table — never trust the
      frontend for this, and never mixed between currencies).
    ...
    table = build_dynamic_prize_table(play_amount, tier_multiplier)
    return {
        "currency": "NGN",
        "play_amount": play_amount,
        "display_segments": WHEEL_DISPLAY_VALUES_NGN,
        "segments": [{"prize": prize, "probability": prob} for prize, prob in table],
    }
...
def _spin_play_ngn(payload, db: Session, user: models.User):
    ...
    tier_label, tier_multiplier = ledger_service.get_user_ngn_deposit_tier(db, user.id)
    outcome = resolve_dynamic_spin(payload.play_amount, tier_multiplier)
    prize = outcome["prize"]
